...
// Unicode block progress bar: filled (█) proportional to pct, padded with ░.
const BAR_WIDTH = 10;
function usageBar(pct, width = BAR_WIDTH) {
  const filled = Math.round((clampPct(pct) / 100) * width);
  return '█'.repeat(filled) + '░'.repeat(width - filled);
}

// "<emoji> <label> ██████░░░░ NN%" for a usage metric (ctx / 5h / 7d). The bar
// fills toward the limit and the whole segment is colored by how close it is:
// >=80% red, >=50% yellow, otherwise the terminal default.
function usageSegment(emoji, label, pct) {
  const p = clampPct(pct);
  const text = `${label} ${usageBar(p)} ${p}%`;
  const code = p >= 80 ? '91' : p >= 50 ? '33' : null;
  return `${emoji} ${code ? color(code, text) : text}`;
}

// Second line (quota): context-window / 5h / weekly usage bars (or session
// cost) / reset countdown / billing-block cost / burn rate.
function buildQuotaLine(data, cacheText, lang) {
  ...
  // context_window is null on some plans), fall back to stdin.
  let pct = null;
  ...
  if (pct != null) parts.push(usageSegment('🪟', 'ctx', pct));

  // Official subscription quota (rate_limits): 5-hour and weekly usage as bars.
  // These are used_percentage straight from Claude Code — the bar fills toward
  // the throttle. Fall back to ccusage session cost when rate_limits is absent.
  const used = five.used_percentage;
  if (typeof used === 'number') {
    parts.push(usageSegment('🔋', '5h', used));
    const weekUsed = week.used_percentage;
    if (typeof weekUsed === 'number') parts.push(usageSegment('📅', '7d', weekUsed));
  } else {
    const mSess = /(\$[\d.]+)\s+session/.exec(cacheText);
    if (mSess) parts.push(t.session(mSess[1]));
