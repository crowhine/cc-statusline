...
// Billing-block token usage. Unlike the per-session ccusage cache this one is
// machine-wide: the active block is a single global window, so every session
// shares one file (and one refresh).
//
// `ccusage blocks --active` has to walk every transcript on disk, which costs
// ~40 CPU-seconds -- about a thousand times more than the `ccusage statusline`
// call that feeds the rest of this line (~0.04s, it only reads the current
// session). That gap is the entire reason for the TTL, the lock, and the opt-out
// below; without them a few open windows would keep a core busy for a cosmetic
// number.
const BLOCK_TOKENS_FILE = path.join(CACHE_DIR, 'blocktokens.txt');
const BLOCK_TOKENS_LOCK = path.join(CACHE_DIR, 'blocktokens.lock');
const BLOCK_TOKENS_DEFAULT_TTL = 180; // seconds
// Past this age the refresh path is presumed broken (ccusage uninstalled, node
// version manager moved, ...). Hide the segment rather than show a number that
// silently froze hours ago.
const BLOCK_TOKENS_STALE_MS = 30 * 60 * 1000;
// A lock left behind by a killed refresh would wedge the segment shut forever.
const BLOCK_TOKENS_LOCK_STALE_MS = 10 * 60 * 1000;

function blockTokensDisabled() {
  return !!process.env.CC_STATUSLINE_NO_BLOCK_TOKENS;
}

function blockTokensTtlMs() {
  const raw = Number.parseInt(process.env.CC_STATUSLINE_BLOCK_TOKENS_TTL || '', 10);
  const secs = Number.isFinite(raw) && raw > 0 ? raw : BLOCK_TOKENS_DEFAULT_TTL;
  return secs * 1000;
}
...
// Read the cached block-token count. Returns null for anything unexpected --
...
function readBlockTokens() {
  if (blockTokensDisabled()) return null;
  let fd = null;
  try {
    const st = fs.statSync(BLOCK_TOKENS_FILE);
    // A FIFO here would block the read forever; only ever read a real file.
    if (!st.isFile()) return null;
    if (Date.now() - st.mtimeMs > BLOCK_TOKENS_STALE_MS) return null;
    fd = fs.openSync(BLOCK_TOKENS_FILE, 'r');
...
function pruneCaches() {
  ...
  for (const f of files) {
    const isTmp = /^ccusage-.*\.txt\.tmp/.test(f) || /^blocktokens\.txt\.tmp/.test(f);
...
function buildQuotaLine(data, cacheText, lang) {
  ...
  // Money spend: billing-block token usage + cost (or session-cost fallback) +
  // burn rate. The token count is the same billing block the cost refers to --
  // not the context window, which 🧠 ctx already covers.
  const money = [];
  const blockTokens = readBlockTokens();
  if (blockTokens != null) money.push(tcol(C.yellow, `🪙 ${formatTokens(blockTokens)}`));
...
// Refresh the machine-wide block-token cache. Called from the same detached
// process as the ccusage cache refresh, but guarded far more tightly because
// `ccusage blocks --active` is ~1000x more expensive than `ccusage statusline`.
//
// Every open Claude Code window spawns its own refresh, so the claim has to be
// atomic: checking the mtime and then touching the file leaves a window where
// several processes all decide to refresh at once. `mkdir` is atomic on POSIX
// and Windows alike, so it does the claiming; the data file's mtime is left to
// mean exactly one thing -- when the number was last *written* -- which is what
// the foreground staleness check depends on.
function refreshBlockTokens() {
  if (blockTokensDisabled()) return;
  const ttlMs = blockTokensTtlMs();
  try {
    // Reclaim a lock orphaned by a killed refresh before testing freshness.
    try {
      const st = fs.statSync(BLOCK_TOKENS_LOCK);
      if (Date.now() - st.mtimeMs > BLOCK_TOKENS_LOCK_STALE_MS) fs.rmdirSync(BLOCK_TOKENS_LOCK);
    } catch {}

    if (isFresh(BLOCK_TOKENS_FILE, ttlMs)) return;

    fs.mkdirSync(CACHE_DIR, { recursive: true });
    try {
      fs.mkdirSync(BLOCK_TOKENS_LOCK); // throws EEXIST when another refresh holds it
    } catch {
      return;
    }

    try {
      // Double-check under the lock: a process that passed the check above may
      // have been waiting on mkdir while the holder finished a full refresh.
      // Without this, a burst of windows still yields several ccusage runs.
      if (isFresh(BLOCK_TOKENS_FILE, ttlMs)) return;

      const parsed = JSON.parse(runCcusage(['blocks', '--active', '--json', '--offline'], ''));
      const blocks = Array.isArray(parsed && parsed.blocks) ? parsed.blocks : [];
      // An empty `blocks` is the normal "idle for over 5 hours, no active block"
      // state, not a failure -- report 0 rather than leaving the previous
      // block's total on screen as if it were the current one.
      const total = blocks.length ? Number((blocks[0] || {}).totalTokens) || 0 : 0;
      if (!Number.isFinite(total) || total < 0) return;

      const tmp = `${BLOCK_TOKENS_FILE}.tmp.${process.pid}`;
      try {
        fs.writeFileSync(tmp, String(Math.round(total)));
        fs.renameSync(tmp, BLOCK_TOKENS_FILE);
      } finally {
        // Rename consumes tmp on success; this clears it on any failure path so
        // dead temp files don't pile up until the 24h prune.
        try {
          fs.unlinkSync(tmp);
        } catch {}
      }
      ...
      try {
        fs.rmdirSync(BLOCK_TOKENS_LOCK);
      } catch {}
    }
  } catch {
    // ccusage missing, malformed output, unwritable cache: keep the old value.
  }
}
